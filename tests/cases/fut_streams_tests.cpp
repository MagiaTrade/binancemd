...
TEST_CASE("Futures Open/Close Stream", "[futures][streams]") {
  auto manager = bmd::BMDManager::create();

  auto streamID = manager->openAggTradeStream(
      bmd::BinanceServiceType::FUTURES,
      "btcusdt",
      10,
...
TEST_CASE("Futures Reconnection", "[futures][streams][reconnection]")
{
  auto manager = bmd::BMDManager::create();
...
TEST_CASE("Futures Messages", "[futures][streams]")
{
  auto manager = bmd::BMDManager::create();
